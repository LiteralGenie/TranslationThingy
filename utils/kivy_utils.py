...
def get_bubble_coords(im_page, bubble, padding=None):
	bbox= bubble.bbox

	# kivy origin is at bottom left. cv2 origin is at top left.
	pos= [bbox['x'], im_page.height-bbox['y']+im_page.pos[1]]
	size= [bbox['w'], -bbox['h']]
	...
	size_sign= [abs(size[0]) // size[0] , abs(size[1]) // size[1]]
	pos= [-padding + pos[0], padding+pos[1]]
	size= [size_sign[0]*padding*2 + size[0], size_sign[1]*padding*2 + size[1]]
